...
public interface ResultRepository extends JpaRepository<ResultEntity, Long> {
        ...
        @Query("SELECT NEW com.miracle.miraclemorningback.dto.TodayRoutinesDto(ro.routineId, ro.routineName, re.memberEntity.memberName, ro.dayOfWeek, ro.certification, ro.startTime, ro.endTime, re.createdAt, re.doneAt, false) FROM ResultEntity re JOIN re.routineEntity ro WHERE re.memberEntity.memberId = :member_id AND DATE(re.createdAt) = CURRENT_DATE AND re.doneAt IS NULL")
        List<TodayRoutinesDto> getActivatedAndIncompleteRoutines(@Param("member_id") Long memberId);

        // [JPQL] 특정 사용자의 루틴 중 활성화되고 인증된 루틴 기록 조회
        @Query("SELECT NEW com.miracle.miraclemorningback.dto.TodayRoutinesDto(ro.routineId, ro.routineName, re.memberEntity.memberName, ro.dayOfWeek, ro.certification, ro.startTime, ro.endTime, re.createdAt, re.doneAt, true) FROM ResultEntity re JOIN re.routineEntity ro WHERE re.memberEntity.memberId = :member_id AND DATE(re.createdAt) = CURRENT_DATE AND re.doneAt IS NOT NULL")
        List<TodayRoutinesDto> getActivatedAndCompleteRoutines(@Param("member_id") Long memberId);
        ...
        // 요일별 달성률
        @Query("SELECT CEIL(COALESCE(((SELECT COUNT(res.resultId) FROM ResultEntity res WHERE res.memberEntity = :member_entity AND FUNCTION('WEEKDAY', res.createdAt) = :day_of_week AND res.doneAt IS NOT NULL) / COUNT(re.resultId) * 100), 0)) FROM ResultEntity re WHERE re.memberEntity = :member_entity AND FUNCTION('WEEKDAY', re.createdAt) = :day_of_week")
        int getDayOfWeekAchievement(
                        @Param("member_entity") MemberEntity memberEntity,
