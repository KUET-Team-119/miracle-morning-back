...
public interface TodayRoutinesRepository extends JpaRepository<TodayRoutinesEntity, Long> {
    ...
    @Query(value = "SELECT ro.routine_id, ro.routine_name, ro.member_id, ro.strategy, ro.certification, ro.start_time, ro.end_time, re.created_at, re.done_at FROM routine ro JOIN result re ON ro.routine_name = re.routine_name AND ro.member_id = re.member_id WHERE ro.member_id = :member_id AND DATE(re.created_at) = CURRENT_DATE AND re.done_at IS NULL", nativeQuery = true)
